setwd("~/KoborLab/kobor_space/zdong/Population_dataset/MHB_New/Analysis/CMR_analysis/Genotype/PCA")
library(gplots) 
...
g<-ggbiplot(ir.pca, obs.scale = 1, var.scale = 1,#labels = regions,
  ...
  scale_colour_manual(values=c("#E69F00","#009E73"))+coord_fixed(ratio=0.7)
  labs(color="Population",size=12)
#dev.off()
ggsave(plot = g, width = 4, height = 4, dpi = 300, filename = "pca_aml0.1_mQTL.pdf")