setwd("~/KoborLab/kobor_space/zdong/Population_dataset/MHB_New/Analysis/CMR_analysis/Han/")
library(gplots) 
...
legend(0.09,0.9421,bty = "n",text.width=c(0.00,0.429),legend=c("EUR","HAN","AFR"),inset=c(-0.05,-0.1),lty=0,xpd=T,horiz=T,cex=0.85)
dev.off()
...
control<-test[,c(1:54)]
han<-test[,c(63:65)]
case_pca=rbind(case,c(2,2))
control_pca=rbind(control,c(0,0))
han_pca=rbind(han,c(1,1))
test_pca<-data.frame(case_pca,control_pca,han_pca)
...
ir.species[ir.species==2]<-"EUR"
ir.species[ir.species==0]<-"AFR"
ir.species[ir.species==1]<-"EAS"
...
g<-ggbiplot(ir.pca, obs.scale = 1, var.scale = 1,#labels = regions,
  ...
  scale_colour_manual(values=c("#E69F00","#9ebcda","#009E73"))+
  labs(color="Population",size=12)+coord_fixed(ratio=1)
#dev.off()
ggsave(plot = g, width = 4, height = 4, dpi = 300, filename = "pca_aml0.1.pdf")
